<script lang="ts">
  ...
  import { scripts, fetchScripts } from './lib/data';
  import { showToast } from './store';

  let search = '';
  let localStates: boolean[] = [];

  $: {
    if ($scripts.length > 0 && localStates.length !== $scripts.length) {
      localStates = $scripts.map((script) => script.active);
    }
  }
        ...
        <div class="script-card">
          <div class="script-card-header">
            <div>
              ...
              <div class="script-trigger">{script.trigger_text}</div>
            </div>
          ...
          <div class="script-footer">
            <div class="script-status">
              ...
              <span class:status-active={localStates[script.index]} class:status-inactive={!localStates[script.index]}>{localStates[script.index] ? script.status_text : 'Inactive'}</span>
            </div>
